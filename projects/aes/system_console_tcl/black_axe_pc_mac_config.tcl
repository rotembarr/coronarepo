...
proc phy_addr_to_mac_addr {addr} {
	global MAC_PC_BASE_ADDR;
	expr {[expr {$addr + 0xA0 + ($MAC_PC_BASE_ADDR / 4)}] * 4};
}
...
# Configure the MAC
master_write_32 $JTAG_MASTER [expr $MAC_0_ADDRESS + $MAC_PC_BASE_ADDR]				$MAC_0;
master_write_32 $JTAG_MASTER [expr $MAC_1_ADDRESS + $MAC_PC_BASE_ADDR]				$MAC_1;
master_write_32 $JTAG_MASTER [expr $MDIO_ADDR1_ADDRESS + $MAC_PC_BASE_ADDR]		$PHY_MDIO_ADDR_ARR(0);
# PCS Configuration
#master_write_32 $JTAG_MASTER $LINK_TIMER_ADDRESS		$LINK_TIMER
#master_write_32 $JTAG_MASTER $LLINK_TIMER_ADDRESS		$LLINK_TIMER
master_write_32 $JTAG_MASTER [expr $IF_MODE_ADDRESS + $MAC_PC_BASE_ADDR]			$IF_MODE;
master_write_32 $JTAG_MASTER [expr $PCS_CTRL_REGISTER_ADDRESS + $MAC_PC_BASE_ADDR] $PCS_CTRL_REG;
master_write_32 $JTAG_MASTER [expr $PCS_CTRL_REGISTER_ADDRESS + $MAC_PC_BASE_ADDR] $PCS_CTRL_REG_RST;
...
master_write_32 $JTAG_MASTER [expr $COMMAND_CONFIG_ADDRESS + $MAC_PC_BASE_ADDR]	$COMMAND_CONFIG;
